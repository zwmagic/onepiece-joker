 ...
 *
 * @uthor zhangwenming
 * @date 2016/10/20 19:12
 * version: 1.0
 */
public class ReflectorUtil {

    private static final Map<Class<?>, ReflectorUtil> CACHE_REFLECTOR = new ConcurrentHashMap<Class<?>, ReflectorUtil>();
    ...
    private ReflectorUtil(Class<?> clazz) {
        addGetMethods(clazz);
        addFields(clazz);
        addSetMethods(clazz);
    }
    ...
    public static ReflectorUtil forClass(Class<?> clazz) {
        return new ReflectorUtil(clazz);
    }

    public static ReflectorUtil fromCache(Class<?> clazz) {
        if (!CACHE_REFLECTOR.containsKey(clazz)) {
            CACHE_REFLECTOR.put(clazz, new ReflectorUtil(clazz));
        }
